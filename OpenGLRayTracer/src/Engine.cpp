...
void mouse_callback(GLFWwindow* window, double xpos, double ypos) {
    Engine* engine = static_cast<Engine*>(glfwGetWindowUserPointer(window));
    engine->rotateCamera((float)xpos, (float)ypos);
}
...
Engine::Engine(int width, int height, int samples)
    ...
    window = glfwCreateWindow(windowWidth, windowHeight, "Ray Tracer", NULL, NULL);
    if (!window) {
        glfwTerminate();
...
void Engine::rotateCamera(float newX, float newY) {
    bool anyMovement = newX != lastMouseX || newY != lastMouseY;
    camera.ProcessMouseMovement(lastMouseX - newX, newY - lastMouseY);
    lastMouseX = newX;
    lastMouseY = newY;
    if (anyMovement) {
        frames = 1;
...
void Engine::setSamples(int samples) {
